...
run_test.py
===========
Run a single VLM on N samples from DisasterM3_Bench.
Outputs a rich, machine-readable JSON designed for research papers.
...
from config import MODELS, RESULTS_DIR, TASK_TRACKS
...
def main():
    ...
    for i, item in enumerate(data):
        # Inference
        try:
            pred_raw = ask_vlm(model, proc, item["primary_image_path"], item["question"], hf_id)
        ...
        if "accuracy" in m:
            ...
            print(f"  ✓ BLEU1={m['bleu1']:.3f} ROUGE-L={m['rougeL']:.3f}")
    ...
    for task, tm in agg["by_task"].items():
        ...
        if track == "single_label_mcq":
            ...
            print(f"    {task:40s} [{n:3d}] B1={tm['bleu1']:.3f} RL={tm['rougeL']:.3f}")
